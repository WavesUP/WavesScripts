...
import scorex.account.ValidationError.MissingSenderPrivateKey
import scorex.account.{Account, PrivateKeyAccount, ValidationError}
...
class Wallet(maybeFilename: Option[String], password: String, seedOpt: Option[Array[Byte]]) extends ScorexLogging {
  ...
  def getAccount: PrivateKeyAccount = {
    privateKeyAccounts().head
  }
